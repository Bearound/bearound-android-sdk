...
- `BluetoothManager.stopScanning()` already clears `lastSeenBeacons` and `trackedBeacons`, but the cleanup now also runs from region exit so background broadcast scans don't accumulate stale data.
- `BeaconManager` now emits a falling-edge event when the last beacon expires (via `cleanupExpiredBeacons` periodic timer at `REGION_CLEANUP_INTERVAL_MS = 2s`) — previously the region state could get stuck at `true` after beacons disappeared.
