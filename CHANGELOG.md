...
- **Permission model aligned with Google's beacon guidance: dropped `neverForLocation`, location declared on all versions.** The SDK is a proximity/beacon SDK — it uses scan results to derive physical location. `BLUETOOTH_SCAN` no longer asserts `neverForLocation` (Android's docs warn that assertion *can filter beacons out of the scan results*), and `ACCESS_FINE/COARSE_LOCATION` are now declared on every version (were `maxSdkVersion=30`, Android ≤ 11 only). On Android 12+ the scan still starts on `BLUETOOTH_SCAN` alone (the gate is unchanged); keeping location granted alongside it is what guarantees full beacon coverage across OEMs, and the Quick Start now requests both. **Data Safety note:** embedding the SDK now means declaring **Location** in your app's Play Data Safety form — foreground only (no `ACCESS_BACKGROUND_LOCATION`), so it does not trigger the background-location review. The three manifests (sdk, app, BearoundScan) are aligned.
